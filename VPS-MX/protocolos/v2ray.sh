...
addusr() {
    ...
    echo -ne "\033[1;32m [1] > " && msg -azu "$(fun_trans "AGREGAR UUID AUTOMATICAMENTE") "
    echo -ne "\033[1;32m [2] > " && msg -azu "$(fun_trans "AGREGAR UUID MANUALMENTE") "
...
addusr_auto() {
    ...
    msg -bar
    msg -tit
    msg -ama "          AGREGAR USUARIO | UUID AUTOMÁTICO"
    msg -bar
    
    ## CORREO Y UUID AUTOMÁTICO
...
addusr_manual() {
    ...
    msg -bar
    msg -tit
    msg -ama "          AGREGAR USUARIO | UUID MANUAL"
    msg -bar
    ...
    while true; do
        echo -ne "\e[91m >> Digita el UUID Manual:\033[1;92m " && read UUID
        # Validación simple para UUID: longitud y formato
        if [[ -z $UUID ]]; then
            err_fun 17 && continue
        # Aunque un UUID es de 36 caracteres, este es un chequeo básico.
        elif [[ "${#UUID}" -lt "30" ]]; then 
            msg -verm "UUID demasiado corto o inválido."
            sleep 2s
            tput cuu1
            tput dl1
            tput cuu1
